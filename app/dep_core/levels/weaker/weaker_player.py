...
class WeakerPlayer:
    ...
    def apply_height_adjustments(self, player_stats):
        ...
        for stat in athletic_stats:
            athletic_stats[stat] = max(1, min(99, athletic_stats.get(stat, 55)))
    ...
    def additional_calculations(self, player_stats):
        ...
        # Balancing logic
        speed_stats = [
            athletic_stats.get('Speed', 35),
            athletic_stats.get('Acceleration', 35),
            athletic_stats.get('Dribbling', 35)
        ]
        heading_stats = [
            athletic_stats.get('Jumping', 35),
            athletic_stats.get('Heading', 35)
        ]

        if all(stat < 55 for stat in speed_stats):
            athletic_stats['Jumping'] = max(athletic_stats['Jumping'], 50)
            athletic_stats['Heading'] = max(athletic_stats['Heading'], 50)
        elif all(stat < 55 for stat in heading_stats):
            athletic_stats['Speed'] = max(athletic_stats['Speed'], 50)
            athletic_stats['Acceleration'] = max(athletic_stats['Acceleration'], 50)
            if self.position not in ['Defender', 'Goalkeeper', 'FullBack']:
                athletic_stats['Dribbling'] = max(athletic_stats['Dribbling'], 45)
            else:
                athletic_stats['Dribbling'] = max(athletic_stats['Dribbling'], 40)

        # Adjust Physical Power
        if athletic_stats['Jumping'] > 50:
            athletic_stats['Physical Power'] = max(athletic_stats['Physical Power'], 50)

        if athletic_stats['Acceleration'] < 42 or athletic_stats['Speed'] < 42:
            athletic_stats['Physical Power'] = self.generate_stat(55, 4)
        ...
        if attack_stats and set_pieces:
            if attack_stats.get('Finishing', 0) > 55 or attack_stats.get('Off The Ball', 0) > 55:
                if 'Penalty' in set_pieces:
                    set_pieces['Penalty'] += 5

            if attack_stats.get('Long Shots', 0) > 55 or playmaking_stats.get('Passing', 0) > 55:
