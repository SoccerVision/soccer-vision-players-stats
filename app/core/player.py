...
class Player:
    ...
    def merge_configs(self, base_config, position_config):
        for category, stats in position_config.items():
            if category in base_config:
                base_config[category].update(stats)
            else:
                base_config[category] = stats
        return base_config
