...
class WeakerPlayer:
    ...
    def assign_name(self) -> str:
        first_names = ['Gary', 'Kevin', 'Frank', 'Ted', 'Paul', 'Charlie', 'Stanley', 'Norman', 'Alan', 'Greg']
        last_names = ['Carter', 'Adams', 'Parker', 'Evans', 'Baker', 'Davis', 'Scott', 'Collins', 'Morris', 'Wright']
    ...
    def get_stat_config(self):
        stat_config = {
            'Mental': {
                ...
                'Teamwork': {'mean': 30, 'std_dev': 8},
                'Decisions': {'mean': 30, 'std_dev': 6},
                'Composure': {'mean': 30, 'std_dev': 6},
            },
            'Defense': {
            ...
            'Attack': {
                ...
                'Long Shots': {'mean': 30, 'std_dev': 4},
                'Off The Ball': {'mean': 30, 'std_dev': 4},
            },
            'Playmaking': {
                'Creative': {'mean': 30, 'std_dev': 4},
                'Passing': {'mean': 30, 'std_dev': 4},
                'Crossing': {'mean': 30, 'std_dev': 4},
            },
            'Athletic': {
                'Physical Power': {'mean': 35, 'std_dev': 8},
                'Ball Control': {'mean': 40, 'std_dev': 4},
                'Dribbling': {'mean': 40, 'std_dev': 4},
                'Jumping': {'mean': 35, 'std_dev': 8},
                'Acceleration': {'mean': 40, 'std_dev': 8},
                'Speed': {'mean': 40, 'std_dev': 8},
            },
            'Set Pieces': {
                'Free Kicks': {'mean': 10, 'std_dev': 15, 'min': 30, 'max': 90},
                'Penalty': {'mean': 40, 'std_dev': 15, 'min': 30, 'max': 90},
            },
        }
        return stat_config
    ...
    def additional_calculations(self, player_stats):
        ...
        # Balancing logic
        speed_stats = [
            athletic_stats.get('Speed', 30),
            athletic_stats.get('Acceleration', 30),
            athletic_stats.get('Dribbling', 30)
        ]
        heading_stats = [
            athletic_stats.get('Jumping', 30),
            athletic_stats.get('Heading', 30)
        ]

        if all(stat < 50 for stat in speed_stats):
            athletic_stats['Jumping'] = max(athletic_stats['Jumping'], 45)
            athletic_stats['Heading'] = max(athletic_stats['Heading'], 45)
        elif all(stat < 50 for stat in heading_stats):
            athletic_stats['Speed'] = max(athletic_stats['Speed'], 45)
            athletic_stats['Acceleration'] = max(athletic_stats['Acceleration'], 45)
            if self.position not in ['Defender', 'Goalkeeper', 'FullBack']:
                athletic_stats['Dribbling'] = max(athletic_stats['Dribbling'], 40)
            else:
                athletic_stats['Dribbling'] = max(athletic_stats['Dribbling'], 35)

        # Adjust Physical Power
        if athletic_stats['Jumping'] > 45:
            athletic_stats['Physical Power'] = max(athletic_stats['Physical Power'], 45)

        if athletic_stats['Acceleration'] < 37 or athletic_stats['Speed'] < 37:
            athletic_stats['Physical Power'] = self.generate_stat(50, 4)
        ...
        if attack_stats and set_pieces:
            if attack_stats.get('Finishing', 0) > 50 or attack_stats.get('Off The Ball', 0) > 50:
                if 'Penalty' in set_pieces:
                    set_pieces['Penalty'] += 5

            if attack_stats.get('Long Shots', 0) > 50 or playmaking_stats.get('Passing', 0) > 50:
